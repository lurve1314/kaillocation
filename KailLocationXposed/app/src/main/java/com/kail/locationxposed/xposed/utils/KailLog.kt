...
object KailLog {
    ...
    fun log(context: Context?, tag: String, message: String, isHighFrequency: Boolean = false, level: Char = 'd') {
        ...
        // 如果日志开关未开启，则不进行任何操作
        if (!logEnabled || !enableLogFromConfig(level)) return
        ...
        // 只有低频日志才保存到公共目录
        if (!isHighFrequency && fileLogEnabled) {
            saveLogToPublicFile(fullTag, fullMessage, level)
        }
    }

    private fun enableLogFromConfig(level: Char): Boolean {
        return detailedLogEnabled || level.lowercaseChar() != 'd'
    }
